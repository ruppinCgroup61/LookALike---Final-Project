  ...
  const fetchItems = async () => {
    try {
      const itemsResponse = await fetch(
        `https://localhost:7215/api/Item/GetAllItemsByUser${email}`,
        {
          method: "GET",
          headers: {
      ...
      const likesResponse = await fetch(
        `https://localhost:7215/api/Algorithm/GetAllLikedItemsByFriend?AdminUserMail=${likerEmail}&ClosetOwnerMail=${email}`
      );
  ...
  const handleLikeToggle = async (itemId, isCurrentlyLiked) => {
    try {
      if (isCurrentlyLiked) {
        // Remove like
        await fetch(
          `https://localhost:7215/api/Algorithm/RemoveLike?adminUserMail=${likerEmail}&closetUserMail=${email}&itemId=${itemId}`,
          {
            method: "DELETE",
          }
        );
        ...
        // Add like
        await fetch(
          "https://localhost:7215/api/Algorithm/LikeItemFromFriendCloset",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
