...
const AllFriends = () => {
    ...
    const fetchFollowers = async () => {
      try {
        const response = await fetch(
          `https://proj.ruppin.ac.il/cgroup61/test2/tar1/api/UserFollower/GetUserFriendsList/${userEmail}`,
          {
            method: "GET",
            headers: {
  ...
  const deleteFriend = async (friendEmail) => {
    try {
      const response = await fetch(
        `https://proj.ruppin.ac.il/cgroup61/test2/tar1/api/UserFollower/deleteFriend?adminMail=${encodeURIComponent(
          userEmail
        )}&friendMail=${encodeURIComponent(friendEmail)}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      setFollowers(
        followers.filter((follower) => follower.following_Email !== friendEmail)
      );
    } catch (error) {
            ...
            {followers.map((follower, index) => (
              <div key={index} className="all-friend-card-container">
                ...
                <div
                  ...
                  onClick={() => deleteFriend(follower.following_Email)}
                >
                  <FontAwesomeIcon
                    icon={faTimes}
                    className="delete-friend-icon"
                  />
                </div>
              </div>
            ))}
    ...
    <>
      <div className="all-friends-container">
        <div className="AF_Header">
          ...
          <h2>All Friends</h2>
          <p id="p_no_friends">You have not any friends yet</p>
        </div>
        <div>
          <NaviBarFooter />
        </div>
