...
function MyWardrobe() {
  ...
  useEffect(() => {
    fetch(`https://proj.ruppin.ac.il/cgroup61/test2/tar1/api/Item/GetAllItemsByUser${userEmail}`, {
      method: "GET",
      headers: {
  ...
  const toggleFavorite = (index) => {
    ...
    let api = `https://proj.ruppin.ac.il/cgroup61/test2/tar1/api/Item/${selectedItem.item_ID}`;
  ...
  const handleDelete = async (item) => {
    try {
      const response = await fetch(
        `https://proj.ruppin.ac.il/cgroup61/test2/tar1/api/Item/DeleteItem?itemId=${item.item_ID}`,
        {
          method: "PUT",
        }
      );
